...
fn compute_feats(
    ...
    let lower_wick_body = if body != 0.0 {
        Some((cur.open.min(cur.close) - cur.low) / body)
    } else {
        None
    };
    let upper_wick_body = if body != 0.0 {
        Some((cur.high - cur.open.max(cur.close)) / body)
    } else {
        None
    };
    let body_ratio = {
        let r = cur.high - cur.low;
        if r != 0.0 {
            Some(body / r)
        } else {
            None
...
static RULES: &[Rule] = &[
    ...
    (
        false,
        [(10, 0, 1.242274122), (26, 2, 3.0), (25, 1, 2.098463339)],
    ),
    ...
    (
        true,
        [(0, 1, 2.325581395), (21, 2, 11.0), (31, 0, -99.31370042)],
    ),
